$(function(){
    ...
    $.ajax({
        ...
        data:{
             endDate: $.getGlobalVal().curDate, //结束时间，当前时间
              pageNo: 'pcWomanFloorBanner', //广告位置
              isValid: 1,//是否有效
        },
        async: false,
        success: function(res){
            ...
            for (var i = 0; i < fBList.length; i++) {
               $('.wo-main').find('.floor .left').eq(i).html('<a target="_blank" href="'+fBList[i].advertisementUrl+'"><img src="'+imgUrl(fBList[i].advertisementPhoto)+'" ></a>')
            }
        }
    })
    ...
    $.ajax({
        ...
        data: {
            endDate: $.getGlobalVal().curDate, //结束时间，当前时间
            pageNo: 'pcWomanProduct', //商品推荐位置
            isValid: 1,//是否有效
        }
    }).done(function(res){
        // console.log('女性楼层名称：',res);
        if( !res.success || res.data.length===0 ){
            return false;
        }
        ...
        for (var i = 0; i < floorList.length; i++) { // 楼层
            ...
            $.ajax({
                ...
                // console.log('女性商品id 楼层 '+i,resTwo);
                ...
                $.ajax({
                    ...
                    // console.log('女性商品详情 楼层 '+i,resPro)
                    ...
                    $('.wo-main .floor').eq(i).find('.right-top').append(topItem)
                    ...
                    $('.wo-main .floor').eq(i).find('.right-bottom').append(bottomItem)
