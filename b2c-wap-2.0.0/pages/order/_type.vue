...
export default {
	...
	watch: {
		selected (){
			...
			if( curSelected == 1 ){
				this.$options.methods.getAllList( this ); //全部订单
			}else if( curSelected == 2 ){
				this.$options.methods.getList( this, JSON.stringify([1,9]) ); //待付款
			}else if( curSelected == 3 ){
				this.$options.methods.getList( this, JSON.stringify([2,3]) ); //待发货
			}else if( curSelected == 4 ){
				this.$options.methods.getList( this, JSON.stringify([4]) ); //待收货
			}else if( curSelected == 5 ){
				this.$options.methods.getList( this, JSON.stringify([5,6,7]) ); //已完成
			}else if( curSelected == 6 ){
				this.$options.methods.getList( this, JSON.stringify([8]) ); //已取消
			}
		},
	},
    mounted(){
		...
		if( this.pid ){
			this.selected = String(this.pid);
			// this.$options.methods.getList( this, this.pid );
			return false;
		}
		switch ( orderType )
		{
			...
			case 'pay':
				this.selected = '2';
				// this.$options.methods.getList( _self, JSON.stringify([1,9]) ); //待付款
				break;
			case 'deliver':
				this.selected = '3';
				// this.$options.methods.getList( _self, JSON.stringify([2,3]) ); //待发货
				break;
			case 'receive':
				this.selected = '4';
				// this.$options.methods.getList( _self, JSON.stringify([4]) ); //待收货
				break;
			case 'complete':
				this.selected = '5';
				// this.$options.methods.getList( _self, JSON.stringify([5,6,7]) ); //已完成
				break;
			case 'cancel':
				this.selected = '6';
				// this.$options.methods.getList( _self, JSON.stringify([8]) ); //已取消
				break;
			default :
    ...
    methods: {
	    ...
	    jumpDetail ( data, index ){
	    	...
	    	this.$router.push({ path: '/order/'+ this.$route.params.type +'/order_detail'});
